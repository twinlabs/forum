...
var User = rootRequire('models/UserLegacy');
...
describe('models/User.js', function() {
  ...
  it('can have an array of posts...', function() {
    ...
    assert(
      user.posts.length === 0,
      "newly-initialized user doesn't have an empty collection of posts",
    );
    ...
    assert(
      user.posts.length === POSTS_SIZE,
      "user doesn't have the number of posts expected",
    );
  });
  ...
  it('has a default displayname', function() {
    ...
    assert(
      ...
      "user's displayname is different from expected",
    );
  });
});
